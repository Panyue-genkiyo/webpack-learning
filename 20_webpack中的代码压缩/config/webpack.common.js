...
const commonConfig = (pd) => ({
    ...
    rules:[
      {
        ...
        sideEffects: true, //开启css模块化 只要引进了css就都是存在副作用的
      },
      {
        test: /\.jsx?$/i,
  ...
  plugins:[
    new HtmlWebpackPlugin({
      ...
      cache:true, //当html文件再次打包与前一次一样的话将直接使用缓存
      minify: pd ? {
          removeComments:true,
          removeRedundantAttributes: true, //移除多余的属性
          removeEmptyAttributes:true,  //移除值为空的属性
          collapseWhitespace: true, //折叠空格
          //removeStyleLinkTypeAttributes: true
          minifyCSS:true, //对在html的css进行压缩
          minifyJS: {
            mangle: {
              toplevel: true  //开启压缩内置js代码且丑化它
            }
          }, //同样在压缩在html的模板的内置js代码
       } : false //开发环境下不不压缩，生产环境下压缩的
    }),
    new VueLoaderPlugin(),
